...
def guardian_configured() -> bool:
    """On by default; set OMEN_GUARDIAN=0 to skip the screen (it then degrades to unavailable)."""
    return os.environ.get("OMEN_GUARDIAN", "1").strip().lower() not in {"0", "false", "no", ""}
...
_GEMINI_BACKENDS = {"gemini", "google"}
...
class Guardian:
    ...
    def groundedness(self, *, context: str, response: str) -> dict:
        ...
        if self.use_gemini:
            return self._groundedness_gemini(context=context, response=response)
        if self.use_agent:
    ...
    def _groundedness_gemini(self, *, context: str, response: str) -> dict:
        """Groundedness judgement via Gemini (same writer backend family)."""
        from omen.llm import GeminiLLM, LLMError
        ...
        user = f"### Document(s)\n{context}\n\n### Text to judge\n{response}"
        try:
            raw = GeminiLLM(model=self.model).generate(system=system, user=user)
    ...
    def _groundedness_agent(self, *, context: str, response: str) -> dict:
        ...
        from omen.llm import ClaudeAgentLLM, LLMError
...
def make_guardian() -> Guardian:
    ...
    if backend in _GEMINI_BACKENDS:
        from omen.llm import DEFAULT_GEMINI_MODEL

        model = os.environ.get("OMEN_GUARDIAN_MODEL", "") or os.environ.get(
            "OMEN_MODEL", DEFAULT_GEMINI_MODEL
        )
        return Guardian(model=model, use_gemini=True)
    if backend in _AGENT_BACKENDS:
        model = os.environ.get("OMEN_GUARDIAN_MODEL", "")
        if not model or model == DEFAULT_GUARDIAN_MODEL:
            model = os.environ.get("OMEN_MODEL", "sonnet")
        return Guardian(model=model, use_agent=True)
    return Guardian(model=os.environ.get("OMEN_GUARDIAN_MODEL", DEFAULT_GUARDIAN_MODEL))
