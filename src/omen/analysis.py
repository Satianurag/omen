...
ANALYSIS_SYSTEM = (
    ...
    "square brackets exactly as given (for example [k6 run_script], [SigNoz correlate]). Ground "
        ...
        if te := findings.get("top_error"):
            out.append(
                (
                    ...
                    "SigNoz root-cause query",
                )
            )
    if anomalies:
        out.append(
            (
                f"SigNoz p95 time_series forecast {_fmt(anomalies.get('forecast_p95_ms'), ' ms')} over "
                f"{_fmt(anomalies.get('buckets_analyzed'))} buckets; "
                f"{_fmt(anomalies.get('anomalous_buckets'))} rising bucket(s)",
                "SigNoz aggregate_traces",
            )
        )
    if preflight:
        out.append(
            (
                f"SigNoz service {(preflight.get('service') or '?')}, "
                f"{_fmt(preflight.get('span_count'))} spans in preflight window",
                "SigNoz preflight",
            )
        )
    return out
...
def compose_analysis(
    ...
    *,
    ...
    return (
        ...
        f"{evidence_lines}\n\n"
        "Recommendation\n"
        f"{recommend(findings)} Re-run omen after the fix to confirm the regression clears."
    )
