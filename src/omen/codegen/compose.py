...
from omen.codegen.slots import Plan
...
_OPTIONS_BY_TAXONOMY = {
    ...
    "load": (
        "scenarios: {{ stress: {{ executor: 'constant-arrival-rate', rate: 120, "
        "timeUnit: '1s', duration: '30s', preAllocatedVUs: 80, maxVUs: 200, "
        "gracefulStop: '3s' }} }}"
    ),
}
...
def _sample_for_schema(spec: dict, schema: dict, depth: int = 0) -> Any:
    ...
    if t == "string":
        ...
        if fmt == "email":
            return "omen@example.test"
        if fmt == "date-time":
            return "2026-01-01T00:00:00Z"
        if fmt == "password":
            return "omen-passw0rd"
        return "omen"
    return None
...
def compose(*, plan: Plan, openapi_spec: dict | None, endpoints: list[Endpoint], base_url: str) -> str:
    ...
    parts += [
        ...
        "",
        *blocks,
        f"  sleep({_SLEEP_BY_TAXONOMY.get(plan.test_taxonomy, '0')});",
        "}",
    ]
    return "\n".join(parts) + "\n"
