"""``drive_granite``: let a local Granite model drive the omen FSM, step by step.
...
from omen.llm import DEFAULT_MODEL, DEFAULT_NUM_CTX
...
async def drive_granite(
    ...
    *,
    ...
    """Run Granite against a mounted omen server until the FSM is terminal or the cap is hit.
    ...
    takes over. The run's entry parameters (which repo, which target, which SigNoz service) are operator
