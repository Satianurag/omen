"""Unit tests for `_verdict`, locking in the two failure modes omen-bench surfaced:
...
from omen.app import _LATENCY_FLOOR_MS, _verdict
...
def test_server_regression_without_root_cause_message() -> None:
    v = _verdict(
        _state(
            {
                "worst_path": {"path": "/api/visits", "err_pct": 71},
                "top_error": None,
                "server_errors": 8032,
                "client_errors": 0,
                "total_events": 22524,
    ...
    assert v.startswith("server-side regression")
    assert "/api/visits" in v and "server errors under load" in v


def test_high_latency_without_errors() -> None:
    v = _verdict(
        _state(
            {
                "worst_path": {"path": "/api/checkout", "err_pct": 0},
                "top_error": None,
                "server_errors": 0,
                "client_errors": 0,
                "total_events": 4604,
